...
class restore_surveyprofield_integer_subplugin extends restore_subplugin {
     ...
     */
    public function process_surveypro_integer_config($data) {
        $data = (object)$data;
        print_object($data); // Nothing to do, just print the data

        // Just to check that the whole API is available here
        $this->set_mapping('surveypro_integer_config', 1, 1, true);
        $this->add_related_files('mod_surveypro', 'intro', 'surveypro_integer_config');
        print_object($this->get_mappingid('surveypro_integer_config', 1));
    ...
    /**
     * This method processes the submission_config element inside one integer surveypro (see integer subplugin backup)
     */
    public function process_surveypro_integer_submission_config($data) {
        $data = (object)$data;
        print_object($data); // Nothing to do, just print the data
    }
}
