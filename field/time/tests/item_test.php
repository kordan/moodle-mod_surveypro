...
/**
 * Unit tests for surveyprofield_time item
 *
...
use surveyprofield_time\tests\time_item_test_helper;

/**
 * Unit tests for surveyprofield_time\item methods.
 */
#[\PHPUnit\Framework\Attributes\CoversClass(\surveyprofield_time\item::class)]
final class item_test extends \advanced_testcase {
    ...
    /**
     * Instantiate time_item_test_helper with minimal dependencies.
     *
     * @return time_item_test_helper
     */
    private function make_item(): time_item_test_helper {
        $this->resetAfterTest();
        $this->setAdminUser();
        ...
        return new time_item_test_helper($cm, $surveypro, 0, false, true);
    }

    /*------------------------------------------------------------------------
    Tests for item_time_to_unix_time().
    ------------------------------------------------------------------------*/

    /**
     * item_time_to_unix_time() must return an integer.
     */
    public function test_item_time_to_unix_time_returns_integer(): void {
        $item = $this->make_item();
        $result = $item->item_time_to_unix_time(10, 30);

        $this->assertIsInt($result);
    }

    /**
     * item_time_to_unix_time() must be reversible via item_split_unix_time().
     */
    public function test_item_time_to_unix_time_reversible(): void {
        $item = $this->make_item();
        $unixtime = $item->item_time_to_unix_time(10, 30);
    ...
    /**
     * item_time_to_unix_time() for different times must return different values.
     */
    public function test_item_time_to_unix_time_different_values(): void {
        $item = $this->make_item();
        $time1 = $item->item_time_to_unix_time(0, 0);
        $time2 = $item->item_time_to_unix_time(23, 59);
    ...
    public function test_get_multilang_fields_has_expected_keys(): void {
        ...
        $this->assertArrayHasKey('surveyprofield_time', $result);
    }
