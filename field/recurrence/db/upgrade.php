    ...
    if ($oldversion < 2024022701) {
        ...
        if ($condition) {
            ...
            $sql = 'UPDATE {surveypro_item}
                    SET content = (SELECT f.content FROM {surveyprofield_recurrence} f '.$whereclause.'),
                        contentformat = (SELECT f.contentformat FROM {surveyprofield_recurrence} f '.$whereclause.')
                    WHERE EXISTS (SELECT 1 FROM {surveyprofield_recurrence} f '.$whereclause.')';
            $DB->execute($sql);
        }
    ...
    if ($oldversion < 2024032800) {
        ...
        foreach ($fieldnames as $fieldname) {
            ...
            if ($dbman->field_exists($table, $field)) {
                ...
                $whereclause = 'WHERE f.itemid = {surveypro_item}.id';
                $sql = 'UPDATE {surveypro_item}
                        SET '.$fieldname.' = (SELECT f.'.$fieldname.' FROM {surveyprofield_recurrence} f '.$whereclause.')
                        WHERE EXISTS (SELECT 1 FROM {surveyprofield_recurrence} f '.$whereclause.')';
                $DB->execute($sql);
            }
