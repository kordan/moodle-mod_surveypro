...
function xmldb_surveyprofield_radiobutton_upgrade($oldversion) {
    ...
    if ($oldversion < 2014051701) {
        ...
        // Radiobutton savepoint reached.
        upgrade_plugin_savepoint(true, 2014051701, 'surveyprofield', 'radiobutton');
    }

    if ($oldversion < 2024022701) {

        // Define field content to be dropped from surveyprofield_radiobutton.
        $table = new xmldb_table('surveyprofield_radiobutton');
        ...
        if ($condition) {
            ...
            //         JOIN {surveyprofield_radiobutton} f ON f.itemid = i.id
            ...
            $sql = 'UPDATE {surveypro_item}
                    SET content = (SELECT f.content FROM {surveyprofield_radiobutton} f '.$whereclause.'),
                        contentformat = (SELECT f.contentformat FROM {surveyprofield_radiobutton} f '.$whereclause.')
                    WHERE EXISTS (SELECT 1 FROM {surveyprofield_radiobutton} f '.$whereclause.')';
            $DB->execute($sql);
        }
        ...
        // Radiobutton savepoint reached.
        upgrade_plugin_savepoint(true, 2024022701, 'surveyprofield', 'radiobutton');
    }
    ...
    if ($oldversion < 2024032800) {
        ...
        foreach ($fieldnames as $fieldname) {
            ...
            if ($dbman->field_exists($table, $field)) {
                ...
                $whereclause = 'WHERE f.itemid = {surveypro_item}.id';
                $sql = 'UPDATE {surveypro_item}
                        SET '.$fieldname.' = (SELECT f.'.$fieldname.' FROM {surveyprofield_radiobutton} f '.$whereclause.')
                        WHERE EXISTS (SELECT 1 FROM {surveyprofield_radiobutton} f '.$whereclause.')';
                $DB->execute($sql);
            }
        ...
        // Radiobutton savepoint reached.
        upgrade_plugin_savepoint(true, 2024032800, 'surveyprofield', 'radiobutton');
    }
