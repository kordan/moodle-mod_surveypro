...
use surveyprofield_shortdate\tests\shortdate_item_test_helper;

/**
 * Unit tests for surveyprofield_shortdate\item methods.
 */
#[\PHPUnit\Framework\Attributes\CoversClass(\surveyprofield_shortdate\item::class)]
class item_test extends \advanced_testcase {
    ...
    /**
     * Instantiate shortdate_item_test_helper with minimal dependencies.
     *
     * @return shortdate_item_test_helper
     */
    private function make_item(): shortdate_item_test_helper {
        $this->resetAfterTest();
        $this->setAdminUser();
    ...
    // -------------------------------------------------------------------------
    // Tests for item_shortdate_to_unix_time()
    // -------------------------------------------------------------------------

    /**
     * item_shortdate_to_unix_time() must return an integer.
     */
    public function test_item_shortdate_to_unix_time_returns_integer(): void {
        $item = $this->make_item();
        $result = $item->item_shortdate_to_unix_time(6, 2024);

        $this->assertIsInt($result);
    }

    /**
     * item_shortdate_to_unix_time() must be reversible via item_split_unix_time().
     */
    public function test_item_shortdate_to_unix_time_reversible(): void {
        $item = $this->make_item();
        $unixtime = $item->item_shortdate_to_unix_time(6, 2024);
    ...
    /**
     * item_shortdate_to_unix_time() for different dates must return different values.
     */
    public function test_item_shortdate_to_unix_time_different_dates(): void {
        $item = $this->make_item();
        $date1 = $item->item_shortdate_to_unix_time(1, 2024);
        $date2 = $item->item_shortdate_to_unix_time(12, 2024);
    ...
    /**
     * get_downloadformats() must return an array with 7 entries.
     */
    ...
    public function test_get_multilang_fields_has_expected_keys(): void {
        ...
        $this->assertArrayHasKey('surveyprofield_shortdate', $result);
    }
