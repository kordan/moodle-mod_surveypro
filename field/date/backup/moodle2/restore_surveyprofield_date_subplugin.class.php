...
/**
 * This file contains the restore code for the surveyprofield_date plugin.
 *
 ...
 * to restore one surveyprofield_date subplugin.
 *
 * @package   surveyprofield_date
 * @copyright 2010 onwards Eloy Lafuente (stronk7) {@link http://stronk7.com}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class restore_surveyprofield_date_subplugin extends restore_subplugin {
     ...
     * @param mixed $data
     */
    public function process_surveyprofield_date($data) {
        global $DB;

        $data = (object)$data;
        $data->itemid = $this->get_new_parentid('surveypro_item');

        // Insert the surveyprofield_date record.
        $newdateid = $DB->insert_record('surveyprofield_date', $data);
    }
}
