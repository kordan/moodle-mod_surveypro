...
use surveyprofield_date\tests\date_item_test_helper;

/**
 * Unit tests for surveyprofield_date\item methods.
 */
#[\PHPUnit\Framework\Attributes\CoversClass(\surveyprofield_date\item::class)]
final class item_test extends \advanced_testcase {
    /**
     * Instantiate surveyprofield_date\item with minimal dependencies.
     *
     * @return item
     */
    private function make_item(): date_item_test_helper {
        $this->resetAfterTest();
        $this->setAdminUser();
        ...
        return new date_item_test_helper($cm, $surveypro, 0, false, true);
    }

    /*------------------------------------------------------------------------
    Tests for item_date_to_unix_time().
    ------------------------------------------------------------------------*/

    /**
     * item_date_to_unix_time() must return an integer.
     */
    public function test_item_date_to_unix_time_returns_integer(): void {
        $item = $this->make_item();
        $result = $item->item_date_to_unix_time(2024, 6, 15);

        $this->assertIsInt($result);
    }

    /**
     * item_date_to_unix_time() must be reversible via item_split_unix_time().
     */
    public function test_item_date_to_unix_time_reversible(): void {
        $item = $this->make_item();
        $unixtime = $item->item_date_to_unix_time(2024, 6, 15);
    ...
    /**
     * item_date_to_unix_time() for different dates must return different values.
     */
    public function test_item_date_to_unix_time_different_dates(): void {
        $item = $this->make_item();
        $date1 = $item->item_date_to_unix_time(2024, 1, 1);
        $date2 = $item->item_date_to_unix_time(2024, 12, 31);
    ...
    /**
     * get_friendlyformat() must return strftime05.
     */
    public function test_get_friendlyformat(): void {
        $item = $this->make_item();
        $this->assertEquals('strftime05', $item->get_friendlyformat());
    }
    ...
    /**
     * get_downloadformats() must return an array with 11 entries.
     */
    ...
    public function test_get_multilang_fields_has_expected_keys(): void {
        ...
        $this->assertArrayHasKey('surveyprofield_date', $result);
    }
