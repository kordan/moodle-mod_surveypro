...
class behat_mod_surveypro extends behat_base
{
     ...
     * that are required
     *
     * @Given /^surveypro "([^"]*)" has the following items:$/
    ...
    protected function get_action_icons(\mod_surveypro\itembase $item, array $iconset): string {
        ...
        // Each grid cell corresponds to a fixed column.
        // The widths are defined in CSS (see styles.css below).
        // Here we always render exactly 5 cells, in the following order:
        // - col1: required/optional toggle  (or empty cell)
        // - col2: outdent ◄                 (or empty cell)
        // - col3: value [n]                 (or empty cell)
        // - col4: indent ►                  (or empty cell)
        // - col5: menu ⋮                    (always present)
        $cells = [];

        // ── COL 1: Required toggle / forced-optional / vuoto ─
        ...
        if ($classname::has_mandatoryattribute()) {
            if ($item->item_canbesettomandatory()) {
                ...
                $cells[] = $renderer->render_from_template('core/inplace_editable', $tmpl->export_for_template($renderer));
            } else {
                ...
                $cells[] = $OUTPUT->render($forcedoptionalicn);
            }
        } else {
            $cells[] = ''; // Empty cell: the grid preserves the space.
        }

        // ── COL 2+3+4: Indent controls ───────────────────────
        if ($item->get_insetupform('indent')) {
            ...
            if ($currentindent !== false) {
                ...
                if ($currentindent > 0) {
                    ...
                    $lefticn = new \pix_icon(
                        ...
                        $iconset['outdentstr'],
                        'moodle',
                        ['title' => $iconset['outdentstr'], 'alt' => $iconset['outdentstr'],
                        'aria-hidden' => 'true']
                    );
            ...
            // Item senza indent: tre celle vuote per mantenere la griglia.
            $cells[] = '';
            $cells[] = '';
            $cells[] = '';
        }
        ...
        $cells[] = $OUTPUT->render($menu);

        // ── Grid wrapper ────────────────────────────────
        // Wraps each cell in a <div> tag and uses the
        // .surveypro-actions-grid class defined in styles.css.
        $html = '';
        foreach ($cells as $cell) {
            $html .= \html_writer::tag('div', $cell, ['class' => 'surveypro-actions-cell']);
        }
        return \html_writer::tag('div', $html, ['class' => 'surveypro-actions-grid']);
    }

    /**
     * Opens the action dropdown for the item at the given sortindex
     * and clicks the named action link inside it.
     *
     * Example:
     *   And I click action "Edit" on item 2
     *
     * @When /^I click action "(?P<action>[^"]*)" on item (?P<sortindex>\d+)$/
     *
     * @param string $action
     * @param int $sortindex
     * @return void
