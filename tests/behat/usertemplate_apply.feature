...
Feature: Load and apply usertemplates to test partial item deletion
  ...
  Scenario: Load and apply a usertemplate
    ...
    And I click action "Hide" on item 5
    And I click action "Hide" on item 6
    And I click action "Hide" on item 7
    And I click action "Hide" on item 60
    ...
    And I select "Apply" from the "jump" singleselect
    And I set the following fields to these values:
      | User templates | (This course) parent-child_2024032800.xml |
      | id_action_17   | 1                                         |
    And I press "Apply"
