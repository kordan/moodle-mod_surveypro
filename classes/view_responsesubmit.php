...
class view_responsesubmit extends formbase {
    ...
    private function save_surveypro_submission() {
        ...
        // and will be saved after each respose in function save_user_response.
    ...
    public function save_user_response() {
        ...
        // There are two types of undesired answers.
        ...
        // A check is needed to verify that EACH surveypro field was actually saved as VERIFIED.
        ...
        // 7) So, the submission HAS TO BE saved as IN PROGRESS and not as CLOSED
     ...
     * They comes from answers in pages not verified (because user returned to previous page)
     * Unverified answers are set to 'verified' when the users arrives to the page where they are NOT DISABLED and moves forward.
     * If, at submission time, unverified answers are still somewhere... they must be deleted.
     *
     * Example
     ...
     * Page 2: 'Question2: Do you confirm you are a male?' 'I am a male' and '<< Previous page'
     * 'I am a male' is saved as unverified
     * Page 1: What is your gender? 'F' and 'Next page >>'
     * Page 2: 'Question3: Do you confirm you are a female?' 'I am a female' and '<< Previous page'
     * 'I am a female' is saved as unverified
     * Page 1: What is your gender? 'M' and 'Next page >>'
     * Page 2: 'Question2: Do you confirm you are a male?': 'I confirm I am a male' and 'Submit'
     *
     * At submit time I have:
     * Question1: 'M' is saved as verified
     * Question2: 'I am a female' is saved as unverified
     * Question3: 'I confirm I am a male' is saved as verified
     *
    ...
    public function drop_unverified_data() {
        $utilitylayoutman = new utility_layout($this->cm, $this->surveypro);
        $utilitylayoutman->delete_answers(['submissionid' => $this->formdata->submissionid, 'verified' => 0]);
    }
        ...
        if ($this->mode == SURVEYPRO_READONLYMODE) {
            ...
            if ($userformpagecount > 1) {
                if ($this->formpage > $this->userfirstpage) {
                    ...
                    $backwardbutton = new \single_button($url, get_string('previousformpage', 'mod_surveypro'));
                }
                ...
                if ($this->formpage < $this->userlastpage) {
                    ...
                    $params['formpage'] = $this->get_nextpage();
                    $url = new \moodle_url('/mod/surveypro/view.php', $params);
                    $forwardbutton = new \single_button($url, get_string('nextformpage', 'mod_surveypro'));
                }
     ...
     *
     * The first objective is to drop all the answers returned by disabled items.
     *
     * If a child item is disabled by the parent in its same page,
     * the child answer (whether set) is undesired and must be deleted.
     *
     * First scenario.
     * Let's suppose parent and child element share the same page.
     ...
     * I provide the right parent answer so that the child becomes enabled.
     * I save.
     * Tomorrow I return to the same response and I change my mind so that the child item is no longer enabled.
     * At save time I have to drop the answers provided in the first submission.
     *
     * @return void
     */
    private function drop_undesired_answers() {
        ...
        // Take in mind that, for mysterious motivations, NOT ALWAYS mform returns ONLY enabled element content.
        $submittedformpage = $this->formdata->formpage;

        // I must get the list of ALL the items of this page to check if they were already submitted in the past
        // even if they were not submitted now.
        $canaccessreserveditems = has_capability('mod/surveypro:accessreserveditems', $this->context);
        ...
        foreach ($itemseeds as $itemseed) {
            ...
            if (in_array($itemseed->parentid, $disposelist)) {
                $disposelist[] = $itemseed->id;
                continue;
            }

            // Let's start.
            // Get parentitem class.
            $parentitem = surveypro_get_itemclass($this->cm, $this->surveypro, $itemseed->parentid);

            $parentpage = $parentitem->get_formpage();
            if ($parentpage == $submittedformpage) { // If parent and child are both here in the just submitted page.
                // Pass to parentitem what the child needs to be enabled ($itemseed->parentvalue)
                // and compare it with what was answered to parentitem ($dirtydata).
                if (!$parentitem->userform_is_child_allowed_dynamic($itemseed->parentvalue, $dirtydata)) {
                    // Parentitem, knowing itself, compares the anwer it received with child needs and provides an answer.
                    $disposelist[] = $itemseed->id;
                }
            } else {
                // Parent item is somewhere in the previous pages while the child is here, in the just submitted page.
                $submissionid = $this->formdata->submissionid;
                if (empty($submissionid)) { // The submission is new.
                    // No answers from previous submission can be found. Carry on.
                    continue;
                }

                // Is there an already saved answer for the item $itemseed->id in the frame of the submission $submissionid?
                $sqlparams = ['submissionid' => $submissionid, 'itemid' => $itemseed->id];
                if (!$DB->record_exists('surveypro_answer', $sqlparams)) {
                    continue;
                }

                // Is your parent item still allowing you?
                $parentitem = surveypro_get_itemclass($this->cm, $this->surveypro, $itemseed->parentid);
                if (!$parentitem->userform_is_child_allowed_static($submissionid, $itemseed)) {
                    $utilitylayoutman->delete_answers(['submissionid' => $submissionid, 'itemid' => $itemseed->id]);
                }
            }
        } // Check next item.
